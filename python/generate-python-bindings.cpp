...
PYBIND11_MODULE(eos, eos_module)
{
    ...
    py::class_<morphablemodel::PcaModel>(morphablemodel_module, "PcaModel", "Class representing a PcaModel with a mean, eigenvectors and eigenvalues, as well as a list of triangles to build a mesh.")
        ...
        .def("draw_sample", py::overload_cast<std::vector<float>>(&morphablemodel::PcaModel::draw_sample, py::const_), "Returns a sample from the model with the given PCA coefficients. The given coefficients should follow a standard normal distribution, i.e. not be scaled with their eigenvalues/variances.", py::arg("coefficients"));
    ...
    py::class_<morphablemodel::MorphableModel>(morphablemodel_module, "MorphableModel", "A class representing a 3D Morphable Model, consisting of a shape- and colour (albedo) PCA model, as well as texture (uv) coordinates.")
        ...
        .def("draw_sample", &morphablemodel::MorphableModel::draw_sample, "Returns a sample from the model with the given shape- and colour PCA coefficients.", py::arg("shape_coefficients"), py::arg("color_coefficients"))
    ...
    fitting_module.def("estimate_orthographic_projection_linear",
                       [](std::vector<Eigen::Vector2f> image_points,
                          std::vector<Eigen::Vector4f> model_points, bool is_viewport_upsidedown,
                          int viewport_height) {
                       ...
                       py::arg("image_points"), py::arg("model_points"), py::arg("is_viewport_upsidedown"), py::arg("viewport_height") = 0);
    ...
    fitting_module.def("fit_shape_and_pose",
        [](const morphablemodel::MorphableModel& morphable_model,
           ...
           int num_iterations, int num_shape_coefficients_to_fit, float lambda) {
            ...
            // We can change this to std::optional as soon as we switch to VS2017 and pybind supports
            // std::optional
        ...
        py::arg("morphable_model"), py::arg("blendshapes"), py::arg("landmarks"), py::arg("landmark_ids"), py::arg("landmark_mapper"), py::arg("image_width"), py::arg("image_height"), py::arg("edge_topology"), py::arg("contour_landmarks"), py::arg("model_contour"), py::arg("num_iterations") = 5, py::arg("num_shape_coefficients_to_fit") = -1, py::arg("lambda") = 30.0f);
